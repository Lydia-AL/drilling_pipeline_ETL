...
def process_machine(dm: Dict[str, Any]) -> Dict[str, Any]:
    """Full ETL pipeline for a drilling machine record."""
    dm = remove_useless_data(dm)
    dm = convert_dates(dm)
    dm = convert_miles_to_meters(dm)
    dm = add_contact_info(dm)
    dm = format_machine_id(dm)
    return dm